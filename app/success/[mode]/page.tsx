    ...
    const fetchData = async () => {
      try {
        ...
        if (data.score >= 75) {
          ...
          const proText = `${baseText}<br/><br/>**Их точечная корректировка даёт основной рост видимости**.<br/><br/>Мы отправили вам два PDF-файла на email: <strong>подробный отчёт с разъяснениями и техническое задание для разработчика</strong>. Это готовый план действий, который можно сразу передать в работу и затем проверить результат повторно.<br/><br/>С уважением, команда AI Signal Max.`;

          setSummary(mode === "pro" ? proText : `${baseText}<br/><br/>${quickText}`);
        } else {
          setSummary(
