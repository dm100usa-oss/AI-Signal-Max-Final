    ...
    const fetchData = async () => {
      try {
        ...
        if (data.score >= 75) {
          ...
          // ТОЛЬКО ЗДЕСЬ ЗАМЕНЕН ТЕКСТ ПРОФЕССИОНАЛЬНОЙ ПОЛНОЙ ПРОВЕРКИ
          const proText = `Ваш сайт частично виден для ИИ-систем. <strong>Вы близки к хорошему результату</strong> – достаточно доработать детали, чтобы структура стала понятнее для ИИ. Тогда сайт сможет чаще появляться в рекомендациях, и вы получите больше переходов и обращений.<br/><br/>
Отдельные параметры пока настроены не полностью, из-за этого сайт не всегда попадает в ответы ИИ. <strong>Их точечная корректировка даёт основной рост видимости.</strong><br/><br/>
Мы отправили вам два PDF-файла на email: <strong>подробный отчёт с разъяснениями для владельца сайта и техническое задание для разработчика</strong>. Это готовый план действий, который можно сразу передать в работу и затем проверить результат повторно.<br/><br/>
С уважением, команда AI Signal Max`;

          setSummary(mode === "pro" ? proText : summary); // быстрый режим не меняется
        } else {
          setSummary(
