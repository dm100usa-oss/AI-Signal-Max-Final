  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (data.score >= 75) {
          setSummary(
            "Ваш сайт хорошо виден для ИИ-ассистентов. Большинство параметров настроены корректно, и системы искусственного интеллекта воспринимают ваш сайт как надёжный источник информации. Это помогает клиентам и ИИ-сервисам быстрее находить вас среди конкурентов. Продолжайте периодически проверять настройки — алгоритмы ИИ меняются, и важно сохранять актуальность, чтобы не потерять позиции. Так вы удержите достигнутый результат и сохраните стабильный поток посетителей из ИИ-платформ."
          );
        } else if (data.score >= 40) {
          const baseText = `Ваш сайт частично виден для ИИ-систем. <strong>Вы близки к хорошему результату</strong> — достаточно доработать детали, чтобы структура стала понятнее для ИИ. Тогда сайт сможет чаще появляться в ответах, и вы получите больше переходов и обращений.`;

          const quickText = `В рамках быстрой проверки мы показываем <strong>основные параметры</strong>, которые требуют доработки.<br/><br/>С уважением, команда AI Signal Max.`;

          const proText = `Отдельные параметры пока настроены не полностью, из-за этого сайт не всегда попадает в ответы ИИ. Их <strong>точечная корректировка даёт основной рост видимости</strong>.<br/><br/>Мы подготовили для вас <strong>подробный отчёт с разъяснениями</strong> и техническое задание для разработчика. Это готовый план действий, который можно сразу передать в работу и затем проверить результат повторно.<br/><br/>С уважением, команда AI Signal Max.`;

          setSummary(
            mode === "pro"
              ? `${baseText}<br/><br/>${proText}`
              : `${baseText}<br/><br/>${quickText}`
          );
        } else {
          setSummary(
            "Ваш сайт почти не показывается ИИ-ассистентам. Несколько критически важных параметров отсутствуют или настроены неправильно. Исправление ошибок значительно повысит шанс, что вас начнут находить по релевантным запросам. Сейчас сайт почти не участвует в показах внутри ИИ-платформ и не попадает в их ответы. После базовых исправлений он сможет снова появляться в рекомендациях и привлекать новых клиентов."
          );
        }
      } catch (err) {
