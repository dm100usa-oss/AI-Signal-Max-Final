...
export default function SuccessPage({ params }: { params: { mode: Mode } }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // --- Логика текста под кругом с порогом 75% ---
        if (mode === "pro") {
          if (data.score >= 75) {
            setSummary(
            ...
            setSummary(
              "Ваш сайт частично виден для ИИ-ассистентов. Некоторые параметры требуют улучшения, и из-за этого ваш ресурс не всегда отображается в ответах. После корректировки ключевых настроек его видимость может заметно вырасти. Вы близки к хорошему результату — достаточно доработать детали, чтобы сайт стал понятнее для ИИ-систем. Тогда ваш сайт начнёт появляться в ответах чаще, и вы получите больше переходов и обращений."
            );
          } else {
            setSummary(
  ...
  return (
    <main className="max-w-3xl mx-auto px-6 py-12">
      ...
      {/* Плавное появление блока под кругом */}
      <div
