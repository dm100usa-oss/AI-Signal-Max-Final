...
function getVerdictConclusion(score: number): string {
  ...
  if (score >= 40)
    return `<span style="font-weight:700;">Главный вывод:</span> ИИ-ассистенты могут рекомендовать ваш сайт только частично. По результатам проверки часть параметров работает правильно, однако имеющиеся недочёты снижают шансы на появление в ответах. После устранения замечаний ситуация заметно улучшится.`;
