...
function buildFactorScores(
  ...
  f.meta_robots    = all.meta_robots.passed === false ? 0 : 1; // null = норма (нет запрета)
  f.sitemap_xml    = passedShare("sitemap_xml");
  f.https          = passedShare("https");
  f.x_robots_tag   = all.x_robots_tag.passed === false ? 0 : 1; // null = норма
  ...
  f.canonical      = all.canonical.passed === false ? 0 : all.canonical.passed === true ? 1 : 0.5;
  f.page_404       = all.page_404.passed === true ? 1 : all.page_404.passed === null ? 0.5 : 0;
  ...
  f.services         = bin(hasServices(html));
  ...
  f.direct_answers     = bin(hasFAQ(html)) * contentBase;
  ...
  f.reviews       = bin(hasReviews(html));
  f.experience    = experienceShare(html);
  f.trust_signals = trustShare(html);
  f.authorship    = bin(hasAuthor(html));
  f.social        = bin(hasSocial(html));
...
function hasServices(html: string | null): boolean {
  if (!html) return false;
  const ld = ldBlob(html);
  if (/"(Service|Offer|OfferCatalog)"/i.test(ld)) return true;
  return /(услуг|services|our services|what we do|наши услуги|сервис|тариф|pricing|prdukt|product)/i.test(html);
}

function hasFAQ(html: string | null): boolean {
  if (!html) return false;
  const ld = ldBlob(html);
  if (/"FAQPage"|"Question"/i.test(ld)) return true;
  return /(faq|часто задаваемые|frequently asked|вопрос[\s\-—]*ответ|q&a|вопросы и ответы)/i.test(html);
}

function hasRegion(html: string | null): boolean {
  if (!html) return false;
  const ld = ldBlob(html);
  if (/"(address|areaServed|addressLocality|addressRegion|addressCountry)"/i.test(ld)) return true;
  // телефонный код страны или почтовый паттерн или слова город/регион
  if (/href=["']tel:\+?\d/i.test(html)) return true;
  return /(город|регион|область|street|улиц|город|city|область|district|chicago|москва|зона обслуживания|обслужива|serving|located in|adres|адрес)/i.test(html);
}
...
function factsShare(html: string | null): number {
  ...
  if (signal >= 40) return 1;
  if (signal >= 20) return 0.75;
  if (signal >= 8) return 0.5;
  if (signal >= 3) return 0.25;
  return 0;
}
...
function hasReviews(html: string | null): boolean {
  if (!html) return false;
  const ld = ldBlob(html);
  if (/"(aggregateRating|review|Review|ratingValue)"/i.test(ld)) return true;
  return /(отзыв|review|рейтинг|rating|звёзд|звезд|stars|★|☆|testimonial)/i.test(html);
}
...
// trust-сигналы: лицензии, гарантии, сертификаты, политика
function trustShare(html: string | null): number {
  const text = cleanText(html).toLowerCase();
  if (!text) return 0;
  let s = 0;
  if (/(лиценз|licens|сертификат|certif|аккредит|accredit)/i.test(text)) s += 0.35;
  if (/(гаранти|warrant|guarantee|страхов|insur)/i.test(text)) s += 0.35;
  if (/(политик[а]? конфиденц|privacy policy|условия|terms|публичн[ая]? оферт)/i.test(text)) s += 0.3;
  return Math.min(1, s);
}

function hasAuthor(html: string | null): boolean {
  if (!html) return false;
  const ld = ldBlob(html);
  if (/"author"/i.test(ld)) return true;
  return /rel=["']author["']|(автор|author)[\s:]/i.test(html);
}

function hasSocial(html: string | null): boolean {
  if (!html) return false;
  return /(facebook\.com|instagram\.com|twitter\.com|x\.com|linkedin\.com|youtube\.com|t\.me|vk\.com)/i.test(html);
}
